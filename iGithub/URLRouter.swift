...
struct URLRouter {
    static func viewControllerForURL(url: NSURL) -> UIViewController {
        if url.isGithubURL {
    ...
    private static func viewControllerForGithubURL(url: NSURL) -> UIViewController {
        ...
        if pathComponents.count == 2 {
            ...
            let repo = Mapper<Repository>().map(
                [
